...
document.getElementById("moreButton").addEventListener("click", function(){
	...
	dataRequest2.open("GET", "songs2.json");
	dataRequest2.send();
})
...
//songs[songs.length] = "Sugar Magnolia > by Grateful Dead on the album American Beauty";
...
// Open and Send the XMLHttpRequest for music from ONLY the songs1.json file
//	songs2.json is not loaded until "more" button is clicked
dataRequest.open("GET", "songs1.json");
dataRequest.send();
...
function populateDOM(){
	...
	for(var i = 0; i < songsObj.length; i++){
		...
		songHtmlStructure = songHtmlStructure.replace("SONG_ARTIST", " by: " + songsObj[i].artist);
		songHtmlStructure = songHtmlStructure.replace("SONG_ALBUM", " on the album: " + songsObj[i].album);
		songHtmlStructure = songHtmlStructure.replace("</ul>", "</ul><br><hr>");
